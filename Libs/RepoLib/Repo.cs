...
namespace RepoLib
{
    public class Repo<T> : IRepo<T> where T : DbContext, new()
    {
        ...
        public Task<RepoResponse> CommitAsync() =>
            Task.Run(() =>
            {
                if (_transaction == null)
                    return new RepoResponse { OpStatus = RepoOperationStatus.Failure, Message = "No transactio open." };

                var repoResponse = new RepoResponse { OpStatus = RepoOperationStatus.Success, Message = string.Empty };
                try
                {
                    _transaction.Commit();
                }
        ...
        {
            DisposeObjects();
            return new RepoResponse { OpStatus = RepoOperationStatus.Failure, Message = e.Message };
        }
