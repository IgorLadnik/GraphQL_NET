...
namespace GraphQlServiceTest
{
    ...
    public class Tests
    {
        ...
        public Tests()
        {
            _clientLogin = new HttpClientWrapper(new WebApplicationFactory<LoginService.Startup>().CreateClient());
            _clientGql = new HttpClientWrapper(new WebApplicationFactory<GraphQlService.Startup>().CreateClient(), token);
        }
